...
const contagemRegressiva = () => {
    if (tempoDecorridoEmSegundos <= 0) {
        ...
        const focoAtivo = html.getAttribute('data-contexto') == 'foco'; // Se esse atributo tem o valor 'foco', eu estou com meu foco ativo
        if(focoAtivo){
            const evento = new CustomEvent('FocoFinalizado'); //CustomEvent é uma API do DOM
            document.dispatchEvent(evento); // O evento é disparado quando o 'foco' finalizar
        }
        zerar();
